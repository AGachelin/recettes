...
class fenetre_edition(QWidget):
    def __init__(self, widget_names,all_tags,unites,liste,win):
        ...
        self.source.setPlaceholderText("Source (ne pas spécifier si la source est déjà dans la recette)")
    ...
    def duree(self):
        self.val_duree = (
            QInputDialog.getInt(self, "Durée de préparation", "Nombre d'heures :",int(self.val_duree))[0],
            QInputDialog.getInt(self, "Durée de préparation", "Nombre de minutes :",int((self.val_duree-int(self.val_duree))*60))[0],
        )
    ...
    def duree1(self):
        self.val_duree1 = (
            QInputDialog.getInt(self, "Durée de la cuisson", "Nombre d'heures :",int(self.val_duree1))[0],
            QInputDialog.getInt(self, "Durée de la cuisson", "Nombre de minutes :",int((self.val_duree1-int(self.val_duree1))*60))[0],
        )
    ...
    def duree2(self):
        self.val_duree2 = (
            QInputDialog.getInt(self, "Durée du repos", "Nombre d'heures :",int(self.val_duree2))[0],
            QInputDialog.getInt(self, "Durée du repos", "Nombre de minutes :",int((self.val_duree2-int(self.val_duree2))*60))[0],
        )
    ...
    def note(self):
        self.val_note = QInputDialog.getInt(self, "Note", "Note :",int(self.val_note), min=0, max=10)[0]
        self.btn3.setText(str(self.val_note)+"/10")

    def four(self):
        self.t_four = QInputDialog.getInt(self, "Four", "Température du four :",int(self.t_four))[0]
        self.btn5.setText("Four : " + str(self.t_four)+"°C")


    def nb(self):
        self.val_nb = QInputDialog.getInt(self, "Nombre de personnes", "Nombre de personnes :",int(self.val_nb), min=0)[0]
    ...
    def save(self,win):
        ...
        for i in ing:
            ...
            if i not in liste_ing:
                query.exec(f"""ALTER TABLE recettes ADD COLUMN '{i}' DOUBLE default(0)""")
                query.exec(f"""ALTER TABLE ing_bis ADD COLUMN '{i}' TEXT""")
                #[syst, unite, text1, rho, rapport]
                if l[1]=="":
                    query.exec(f"""insert into ingredients (nom, arbitraire) values ('{i}',True)""")
                else:
                    query.exec(f"""insert into ingredients (nom, {l[0].lower()}) values ('{i}', True)""")
            elif l[3]>0:
                query.exec(f"""update ingredients set {l[0].lower()}=True, rho={l[3]} where nom='{i}'""")
            query.exec(f"""update ingredients set {l[0].lower()}=True where nom='{i}'""")
            if l[4]>0:
                query.exec(f"""alter table unite add column '{l[1]}' double default(0)""")
                query.exec(f"""update unite set {l[1]}={l[4]} where nom='{l[0].lower()}'""")
        ...
        for i in range(self.tableWidget.rowCount()):
            if "..\data" not in i:
                path = "copy " + self.tableWidget.item(i,0).text().replace('/','\\') +f" ..\\data\\{id}"
                os.system(path)
        win.close()
