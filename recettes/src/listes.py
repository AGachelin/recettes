...
class liste_bouttons(QWidget):
    ...
    def add(self,text1="",ok=False):
        if not(ok):
            text, ok = QInputDialog.getItem(self, f"Ajout d'un {self.texte}", f"{self.texte[0].upper()+self.texte[1:]} :", self.liste, editable=True)
        else:
            text=text1
    ...
    def insert(self):
        try:
            ...
            if ok and text and text not in self.liste_tag:
                ...
                self.remove()
                self.liste_tag=self.liste_tag[:current_row]+[text]+self.liste_tag[current_row:]
            elif ok and text:
...
class liste_ingredients(QWidget):
    ...
    def add(self,ing="",lIng=[],ok1=True):
        if ok1:
            text, ok = QInputDialog.getItem(self, "Ajout d'un ingrédient", "Ingrédient :",sorted(list(self.liste.keys())))
            ...
            if ok and text and not text in self.liste_ing:
                if text not in self.liste:
                    ok=False
                    while not(ok):
                        syst, ok = QInputDialog.getItem(self, "Nouvel ingrédient", "Système de mesure de l'ingrédient :",s)
                        if not ok:
                            QMessageBox.about(self,"Nouvel ingrédient","Indiquer le système de mesure")
                    if syst=="Arbitraire":
                        unite,ok=QInputDialog.getText(self, "Ajout d'un ingrédient", "Nom de l'unité arbitraire (laisser vide si c'est le nombre) :")
                    else:
                        ok=False
                        while not(ok):
                            unite, ok = QInputDialog.getItem(self, "Ajout d'un ingrédient", "Unité :",self.unites[syst])
                            if unite=="":
                    ...
                    if self.liste[text]["Arbitraire"]:
                        u.append("Arbitraire")
                    unite, ok = QInputDialog.getItem(self, "Ajout d'un ingrédient", "Unité :",u)
                    if unite=="Arbitraire":
                        syst='Arbitraire'
                        unite, ok = QInputDialog.getText(self, "Ajout d'un ingrédient","Nom de l'unité arbitraire :")
                    elif unite not in u:
                        if len(syst)>0:
                            i=(s.index(syst[0])+1)%2
                            if len(syst)==1 and unite in self.unites[s[i]]:
                                syst=s[i]
                                rho, ok = QInputDialog.getDouble(self, "Ajout d'une unité",f"Masse volumique de l'ingrédient {text} :",min=0.0000000001)
                            else:
                                ok = QMessageBox.question(self, "Nouvelle unité", f"L'unité {unite} est-elle une unité de {s[((i+1)%2)]} ?")
                                if ok==16384 :
                                    rapport, ok = QInputDialog.getDouble(self, "Ajout d'une unité",f"Coefficient entre {unite} et {base[((i+1)%2)]} :",min=0.0000000001)
                                    syst=s[(i+1)%2]
                                else:
                            ...
                            if unite in self.unites["Masse"]:
                                ...
                                ok = QMessageBox.question(self, "Nouvelle unité", f"L'unité {unite} est-elle une unité de masse ?")
                                if ok==16384 :
                                    rapport, ok = QInputDialog.getDouble(self, "Ajout d'une unité",f"Coefficient entre {unite} et g :",min=0.0000000001)
                                    syst='Masse'
                                else:
                                    rapport, ok = QInputDialog.getDouble(self, "Ajout d'une unité",f"Coefficient entre {unite} et mL :",min=0.0000000001)
                                    syst='Volume'
                        ...
                        for i in ['Masse','Volume']:
                            if unite in self.unites[i]:
                                syst=i
                if unite=="":
                    text1, ok1 = QInputDialog.getDouble(self, "Ajout d'un ingrédient", "Quantité:",min=0.0000000001)
                else:
                    text1, ok1 = QInputDialog.getDouble(self, "Ajout d'un ingrédient", "Quantité (en " + unite + ")",min=0.0000000001)
                if ok1 and text1:
                ...
                QMessageBox.warning(self,"Ajout d'un ingrédient","L'ingrédient a déjà été ajouté")
                self.add()
        else:
